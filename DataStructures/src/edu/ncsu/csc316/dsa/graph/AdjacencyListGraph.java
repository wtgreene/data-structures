...
public class AdjacencyListGraph<V, E> extends AbstractGraph<V, E> {
    ...
    public Edge<E> insertEdge(Vertex<V> v1, Vertex<V> v2, E edgeData) {
        ...
        Position<Edge<E>> posOutgoing = origin.getOutgoing().addLast(edge);
        edge.setOutgoingListPosition(posOutgoing);
        
        if ( isDirected() ) {
        	Position<Edge<E>> posIncoming = destination.getIncoming().addLast(edge);
        	edge.setIncomingListPosition(posIncoming);
        }
    ...
    public void removeVertex(Vertex<V> vertex) {
        ...
        if ( isDirected() )
        	for ( Edge<E> edge : incomingEdges(v) )
        		removeEdge(edge);
